...
class Engine:
    ...
    def _continuity_confusion(cls,response,current_message,profile,lore,recent_turns):
        ...
        current_raw=folded(current_message)
        for match in re.finditer(r'\b(?:eu\s+)?sou\s+([^.!?\n;,]{2,60})',current_raw):
            ...
            if len(phrase) < 4 or len(phrase.split()) > 5:
                continue
            if re.search(rf'\bvoce\s+e\s+(?:mesmo\s+)?{re.escape(phrase)}\b',answer):
        ...
        definition_patterns=(
            r'^.*o\s+que\s+voce\s+quer\s+dizer\s+com\s+(.+)
...
,
            r'^.*o\s+que\s+quer\s+dizer\s+com\s+(.+)
    def delete_turn(self,cid,tid):
        turn=self.db.one('SELECT * FROM turns WHERE campaign=? AND id=?',(cid,tid))
        if not turn:
            raise ValueError('Mensagem não encontrada.')
